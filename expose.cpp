...
private:
    ...
    {
        auto self(shared_from_this());
        async_read(control, buffer(buf), [this, self](const boost::system::error_code &ec, size_t bytes_read)
        {
            if (ec)
            {
                return;
...
int main(int argc, char *argv[])
{
    ...
    try {
        ...
        {
            std::cerr << "Environment variable PROXY_HOST should follow the format <server_ip>:<server_port>\n";   
            return 1;
        }
