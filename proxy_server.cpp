...
public:
    Session(tcp::socket *_control, tcp::socket _client)
        : control(_control), client(std::move(_client)), agent_acceptor(context)
    {
    }
    void do_connect_agent()
    {
        ...
        async_write(*control, buffer(buf), [this, self](const boost::system::error_code &ec, size_t size)
                    {
            if (ec || size != 2) {
                return;
            }
            
            agent_acceptor.async_accept([this, self](boost::system::error_code ec, tcp::socket agent)
                         {
                if (ec) {
                    return;
                }
                ...
                auto piper = std::make_shared<depipe>(std::move(client), std::move(agent));
                piper->pipe();
                piper->rpipe();
            }); });
    }
...
{
public:
    Agent(tcp::socket _control)
        : control(std::move(_control)), proxy(context)
    {
    }
    void do_proxy()
    {
        auto self(shared_from_this());
        async_read(control, buffer(buf), [this, self](const boost::system::error_code &ec, size_t size)
                   {
            if (ec || size != 2) {
                return;
            }
            ...
            std::cout << "New proxy request the port " << port << std::endl;
...
private:
    void do_close()
    {
        control.async_read_some(buffer(buf), [this](const boost::system::error_code &ec, size_t size)
                                {
            proxy.close();
            control.close(); });
    }
    void do_accept()
    {
        ...
        proxy.async_accept(
            [this, self](boost::system::error_code ec, tcp::socket client)
            {
                ...
                std::cout << "New connection at " << proxy.local_endpoint().port() << std::endl;
                std::make_shared<Session>(&control, std::move(client))->do_connect_agent();
                do_accept();
            });
    }
