...
class depipe : public std::enable_shared_from_this<depipe> {
 public:
  ...
  void pipe() {
    ...
    src.async_read_some(buffer(buf), [this, self](
      ...
      async_write(dest, buffer(buf, bytes_read),
  ...
  void rpipe() {
    auto self(shared_from_this());
    dest.async_read_some(buffer(rbuf), [this, self](
                                           const boost::system::error_code &ec,
                                           size_t bytes_read) {
      if (ec) {
        dest.close();
      ...
      async_write(src, buffer(rbuf, bytes_read),
                  [this, self](const boost::system::error_code &ec, size_t) {
                    if (ec) {
                      dest.close();
                      src.close();
